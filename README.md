...
One potential issue that caused difficulty in performing the MiTM attack could be related to restrictions on the UW network, which can prevent ARP spoofing. During lab time, you can connect to our private Wi-Fi network using the following credentials

```
Network: INFO-310-NETWORK
Password: secretpw!
```

When conducting security tests, it is often required to create a controlled environment to eliminate uncontrolled variables that can cause inconsistent results. By creating our own local network we are able to set up a more controlled environment that should lead to a higher success rate.

If your previous MITM attack was unsuccessful revisit the steps from the networking branch README here:

https://github.com/uw-areifers/2024-INFO-310-Password-Manager/tree/networking

and complete the MITM attack before you begin the rest of today's lab.
...
4. Now that we have our certificate signing request saved to `localhost.csr`, we need our certificate authority to provide us a signed cert. We have created a CA, `INFO310-RootCA`, who's certificate and key is saved in the `./certs` directory.
    ...
    ```
    openssl x509 -req -extfile <(printf "subjectAltName=DNS:localhost") -days 365 -in localhost.csr -CA INFO310-RootCA.crt -CAkey INFO310-RootCA.key -CAcreateserial -out localhost.crt
    ```
    - Below is a breakdown of each part of this command:
        ...
        - `-CA INFO310-RootCA.crt`: This specifies the filename of the Certificate Authority's Root Certificate that will be used to sign our certificate.
        - `-CAkey INFO310-RootCA.key`: This specifies the filename of the private key associated with the Root CA certificate. The private key is needed to sign our certificate.
...
4. You should now be able to access your web app on port 443 over HTTPS! To verify this, redeploy your docker image and try to access [https://localhost:443](https://localhost:443).
    ...
    Note that our certificate is not valid. This is because the INFO310-RootCA is not trusted by our computer and browser. In lecture, we will learn more about the importance of a trusted third party and how we can allow our machine to trust the INFO310 certificate authority!
