...
Now that we have made our Loggly account and retrieved our customer token, we can add it to our web application! We can see that our developers have already provided us with the logger component in `loggly-logger.php`:
...
1. Paste in your loggly customer token into your .env file, and assign it the `LOGGLY_TOKEN` variable. 

    ```
    LOGGLY_TOKEN: <your_token>
    ```

2. Next, we need to include the logger component in our actual code. That way, it can log certain events of interest. We have already provided you with an example in the `login.php` script. On line 5, you should see the following code:

    ```
    include './components/loggly-logger.php';
    ```

    This does exactly what it says, it 'includes' the logger component so that we can use it in our program. If you look further down, you will see the following code on line 55:

    ```
    $logger->warning("Login failed for username: $username");
    ```

    This writes to our log file a warning whenever someone fails to log in. We added a nice descriptive log that tells us what happened, as well as the `$username` that failed to log in. We can see this log error in loggly. If you click Logs > Log Explorer you should get to a page where you can see all your logs.
